...
export const list = query({
  ...
  handler: async (ctx, args) => {
    await requireValueAccess(ctx, args.secretValueId);
    return await ctx.db
      .query("attachments")
...
export const commit = mutation({
  ...
  handler: async (ctx, args) => {
    const { value, actor } = await requireValueAccess(ctx, args.secretValueId);
...
export const getDownload = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { actor } = await requireValueAccess(ctx, attachment.secretValueId);
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { value, actor } = await requireValueAccess(ctx, attachment.secretValueId);
